...
public class MembersService {
		...
		try {
			...
			if (isMembers) {
				...
				if (req.getPassword().length() < 6) {
					return new BasicRes(ReplyMessage.PASSWORD_AT_LEAST_SIX_YARDS.getCode(), //
							ReplyMessage.PASSWORD_AT_LEAST_SIX_YARDS.getMessage());
				}
				membersDao.registerMember(req.getName(), req.getRegionsId(), normalizedPhone, encoder.encode(req.getPassword()));
			} else {
	            ...
	            // 覆蓋 id = 1 的資料
				membersDao.registerGuest(req.getName(), req.getRegionsId(), normalizedPhone);
			}
		} catch (Exception e) {
			throw e;
		}
	...
	public MembersRes login(LoginMembersReq req) {
		...
		Regions regions = regionsDao.getById(req.getRegionsId());
		if (regions == null) {
			return new MembersRes(ReplyMessage.REGIONS_ID_NOT_FOUND.getCode(), //
	        		ReplyMessage.REGIONS_ID_NOT_FOUND.getMessage());
		}
		String countryCode = regions.getCountryCode();
		// 電話驗證
