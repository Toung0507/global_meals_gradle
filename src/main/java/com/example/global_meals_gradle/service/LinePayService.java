...
public class LinePayService {
	...
	public String getLinePayLink(String orderDateId, String id) {
		...
		String confirmUrl = String.format(
				"https://your-domain.com/api/payment/linepay/confirm?orderDateId=%s&id=%s", orderDateId, id);
		Map<String, String> redirectUrls = new HashMap<>();
		redirectUrls.put("confirmUrl", confirmUrl); // 後端確認扣款的 API
		...
		try {
			...
			if (resBody != null && "0000".equals(resBody.get("returnCode"))) {
				...
				String errorMsg = resBody != null ? (String) resBody.get("returnMessage") : "無回應";
                throw new RuntimeException("LINE Pay 請求失敗: " + errorMsg);
			}
		} catch (Exception e) {
	 ...
	 */
	public void confirmPayment(String transactionId, String orderDateId, String id,
			HttpSession httpSession) {
		// 從資料庫查出真正的金額，不要相信參數傳進來的 amount
		Orders order = ordersDao.getOrderByOrderDateIdAndId(orderDateId, id);
		if (order == null)
			throw new RuntimeException("找不到訂單");

		int dbAmount = order.getTotalAmount().intValue();
		// 1. 準備請求 Body (只需金額與幣別)
		Map<String, Object> body = new HashMap<>();
		body.put("amount", dbAmount);
		...
		try {
			...
			if ("0000".equals(response.getBody().get("returnCode").toString())) {
				...
				req.setTotalAmount(new BigDecimal(dbAmount));
