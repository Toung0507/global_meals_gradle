...
public class OrdersService {
	...
	public CreateOrdersRes createOrders(CreateOrdersReq req) {
		...
		// DateTimeFormatter.ofPattern("yyyyMMdd"): 定義日期格式    .format(...): 把得到的日期轉換成前面定義的格式
        ...
        for (int i = 0; i < maxRetries; i++) {
            try {
                ...
                if (i == maxRetries - 1) {
                    // 如果重試了 10 次都還是失敗，才拋出錯誤。
                    throw new RuntimeException("系統繁忙，請重新結帳");
                }
                
                // 故意讓程式停 0.05 秒，錯開大家的存檔時間，增加下次成功的機率。
	...
	public BasicRes pay(PayReq req) {
		...
		ordersDao.updatePay(req.getId(), req.getOrderDateId(), req.getPaymentMethod(), req.getPaymentMethod(), req.getStatus().name());
