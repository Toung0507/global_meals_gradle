...
public interface PromotionsGiftsDao extends JpaRepository<PromotionsGifts, Integer> {
	...
	@Modifying
	@Transactional
	@Query(value = "insert into promotions_gifts(id, promotions_id, full_amount, gift_product_id, is_active)"
			+ "values (?1, ?2, ?3, ?4, ?5)", nativeQuery = true)
	public void insert(int id, int promotionsId, BigDecimal fullAmount, int giftProductId, boolean active);
