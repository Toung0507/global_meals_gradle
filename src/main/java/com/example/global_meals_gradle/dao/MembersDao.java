...
public interface MembersDao extends JpaRepository<Members, Integer> {
	...
	@Modifying
	@Transactional
	@Query(value = "INSERT INTO members (id, name, regions_id, phone, password, order_count, is_discount, created_at) "
			+ " values (1, ?1, ?2, ?3, NULL, 0, false, CURDATE()) ON DUPLICATE KEY "
			+ " UPDATE name = ?1, regions_id = ?2, phone = ?3, password = NULL, "
			+ " order_count = 0, is_discount = false, created_at = CURDATE()", nativeQuery = true)
	public void registerGuest(String name, int regionsId, String phone);
	
	// 會員註冊
	@Modifying
	@Transactional
	@Query(value = "INSERT INTO members (name, regions_id, phone, password, order_count, is_discount, created_at) "
			+ " values (?1, ?2, ?3, ?4, 0, false, CURDATE())", nativeQuery = true)
	public void registerMember(String name, int regionsId, String phone, String password);
